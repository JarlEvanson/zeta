...
use super::{hash_block, Digest, BLOCK_SIZE, INITIAL_HASH_VALUES};
...
impl Sha512 {
    ...
    pub fn update_bits(
        ...
        message: &[u8],
        mut bit_count: u128,
    ) -> Result<(), UpdateBitsError> {
        if !self
            .total_bitcount
            .checked_add(bit_count)
            .is_some_and(|new_bitcount| new_bitcount <= u128::MAX - (u128::from(BLOCK_SIZE) - 1))
        {
        ...
        while bit_count > 8 {
            ...
            *current_write = storage | message[byte_offset] >> stored_bits;
            ...
            if self.remainder_count == BLOCK_SIZE {
    ...
    pub fn finalize(&mut self) -> Digest {
        let one_byte = self.remainder_count >> 3;
        ...
        self.remainder[one_byte as usize] = (self.remainder[one_byte as usize] | bit) & !(bit - 1);

        let mut first_zero_byte = one_byte + 1;

        let minumum_remaining = self.remainder_count + 1 + 128;

        if minumum_remaining > BLOCK_SIZE {
            self.remainder[first_zero_byte as usize..].fill(0);
        ...
        let bitcount_bytes = &mut self.remainder[128 - 16..];
...
mod test {
    ...
    fn abc() {
        ...
        sha512.update_bits(msg, msg.len() as u128 * 8).unwrap();
